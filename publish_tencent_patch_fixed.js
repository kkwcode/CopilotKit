...
function publishPackage(packagePath, originalName, aliasName, isFirstPackage = false) {
  const packageJsonPath = path.join(packagePath, 'package.json');
  ...
  try {
    ...
    console.log(`📡 发布到 ${aliasName} (tag: patch)...`);
      ...
      for (const pkg of phase.packages) {
        if (packages.includes(pkg)) {
          ...
          if (aliasName) {
            ...
            await publishPackage(packagePath, originalName, aliasName, isFirstPackage);
          }
        }
      }
